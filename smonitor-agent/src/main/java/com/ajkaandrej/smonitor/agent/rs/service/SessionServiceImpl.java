...
public class SessionServiceImpl implements SessionService {
    ...
    @Override
    public List<Session> getSessions(String application) throws ServiceException {
        ConnectorService service = ConnectorServiceFactory.getService();
        Type listType = new TypeToken<List<Session>>() {}.getType();
        List<Session> result = ObjectMapper.getInstance().map(service.getSessions(application), listType);
        return result;
    }
